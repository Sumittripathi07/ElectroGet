const filterReducer = (state, action) => {
  switch (action.type) {
    ...
    case "SET_LIST_VIEW": //This case is used to set the list view
      return {
        //we change the grid_view to false to actvate list view as true
        ...state,
        grid_view: false,
      };
    ...
    case "SORTING_PRODUCTS":
      ...
      const sortingProducts = (a, b) => {
        if (sorting_value === "lowest") {
          //sorting the products based on the selected value
    ...
    case "FILTER_PRODUCTS":
      ...
      if (text) {
        //filtering the products based on the text
      ...
      if (category !== "all") {
        //filtering the products based on the category but if it is === all then we dont need to filter
      ...
      if (company !== "all") {
        //filtering the products based on the company but if it is === all then we dont need to filter
        tempFilterProduct = tempFilterProduct.filter(
      ...
      if (price === 0) {
        ...
        ); //if price === 0 then we dont need to filter
      } else {
        tempFilterProduct = tempFilterProduct.filter(
          (curElem) => curElem.price <= price
        ); //filtering the products based on the price if curPrice < price then only it will be displayed
      }
      return {
        ...state,
    ...
    case "CLEAR_FILTERS":
      return {
        ...state,
        filters: {
          //reset all the filters to default i.e. category-> all, company-> all, color-> all, price-> maxPrice
